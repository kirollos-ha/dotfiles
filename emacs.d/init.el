...
(defvar theme-hooks-alist
  '((doom-opera . ((lambda ()
    ...
    (nord . ((lambda ()
  	       (progn
  		 (set-face-foreground 'font-lock-comment-face "#ccddee")))))
    (doom-zenburn . ((lambda ()
  		       (progn
  ...
  (defun change-theme-nonint (theme)
    ...
    ;; tanto hai già il wrapper, ci vuole poco
    (disable-all-themes)
    (load-theme theme t)
...
(defun prompt-me-a-file (prompt)
  (expand-file-name
               (car (find-file-read-args prompt t)))) ;; car, I dunno why
...
(defmacro defprompt (name &optional prompt command)
  (let ((proc-name (symbol-name name))
    ...
    `(defun ,name ()
       (interactive)
       (let ((file (prompt-me-a-file ,prompt)))
...
(defvar out-image-viewer "pinta")
(defvar out-doc-annotation "xournal")
(defprompt out-pdf "pdf : " out-pdf-reader)
(defprompt out-image "image : " out-image-viewer)
(defprompt out-annotate "annotata : " out-doc-annotation)

;; put these bad boys with some beframe and we rolling in dough
(defprompt mitosis "new emacs frame to : "
  ("emacsclient" "--create-frame"))
(defprompt mitosis-shell "open eshell to : "
  ("emacsclient" "--create-frame" "--eval" "'(eshell)'"))
...
(global-set-key (kbd "C-x C-b") 'beframe-buffer-menu)
...
(local-set-key 'forth-mode (kbd "C-c C-c") 'forth-eval-defun)
